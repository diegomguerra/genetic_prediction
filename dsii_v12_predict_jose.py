"""
DSII V12 — Predicao Cliente Jose (Lineage Learning + Calibration + Shrinkage)
"""
...
OUTPUT_FILE = RESULTS_DIR / 'jose_predictions_v12_DAU_CAL.xlsx'
EXCLUDED_FILE = RESULTS_DIR / 'jose_excluidos_v12.xlsx'
...
def predict_batch(trait, animals):
    ...
    if cal_dau:
        ...
        preds_raw = d0 + d1 * pa_arr + d2 * dev_preds
        # Expand deviation from PA to match expected genetic variance
        preds_v12 = pa_arr + (preds_raw - pa_arr) * var_exp
    elif cal2f:
        c0, c1, c2 = cal2f['c0'], cal2f['c1'], cal2f['c2']
        preds_v12 = c0 + c1 * pa_arr + c2 * dev_preds
    else:
        preds_v12 = pa_arr + dev_preds
...
# ============================================================
print(f"\n{'='*120}")
print(f"  DSII V12 — PREDICAO CLIENTE JOSE (Lineage Learning + Calibration + Shrinkage)")
print(f"{'='*120}")

df = pd.read_excel(INPUT_FILE)
cols = list(df.columns)
print(f"  {INPUT_FILE.name}: {len(df)} animais ({', '.join(cols)})")

animals_dict = {}
for _, row in df.iterrows():
    aid = row[cols[0]]
    pai = str(row[cols[1]]).strip()
    avo = str(row[cols[2]]).strip()
    bis = str(row[cols[3]]).strip() if len(cols) > 3 else ''
    if pai not in ('', 'nan', 'None'):
        animals_dict[aid] = {'pai': pai, 'avo': avo, 'bis': bis}

print(f"  Animais unicos: {len(animals_dict)}")
...
print(f"\n  Predicao V12 cliente Jose concluida!")
