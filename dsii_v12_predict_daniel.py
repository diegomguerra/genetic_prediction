"""
DSII V12 — Predicao Cliente Daniel (Lineage Learning + Calibration + Shrinkage)
"""
import warnings; warnings.filterwarnings('ignore')
import pandas as pd, pickle, csv, re, numpy as np, sys, time
...
RESULTS_DIR = Path('C:/Users/DiegoGuerra/Documents/Projetos/genetic_prediction/dsii_v12_results')

INPUT_FILE_1 = DOWNLOADS / 'cliente daniel.xls'
INPUT_FILE_2 = DOWNLOADS / 'cliente daniel_2.xls'
OUTPUT_FILE = RESULTS_DIR / 'daniel_predictions_v12.xlsx'
EXCLUDED_FILE = RESULTS_DIR / 'daniel_excluidos_v12.xlsx'

# Load V12 models
with open(RESULTS_DIR / 'v12_models.pkl', 'rb') as f: saved_models = pickle.load(f)
with open(RESULTS_DIR / 'v12_sire_profiles.pkl', 'rb') as f: sire_profiles = pickle.load(f)
with open(RESULTS_DIR / 'v12_mgs_profiles.pkl', 'rb') as f: mgs_profiles = pickle.load(f)
with open(RESULTS_DIR / 'v12_calibration.pkl', 'rb') as f: calibration_params = pickle.load(f)
...
def predict_batch(trait, animals):
    ...
    # Apply calibration
    cal = info.get('calibration', {'b0': 0, 'b1': 1})
    b0, b1 = cal['b0'], cal['b1']
    if abs(b1) > 0.01:
        dev_calibrated = (dev_preds - b0) / b1
    else:
        dev_calibrated = dev_preds

    # Apply shrinkage
    lam = info.get('shrinkage_lambda', 1.0)
    preds_v12 = pa_arr + lam * dev_calibrated
    ...
    if info.get('hybrid') == 'hybrid_3way':
        ...
        for j, vi in enumerate(valid_idx):
            ...
            blend = np.array([[preds_v12[j], pred_2stage, pred_anc, pa_arr[j]]])
            final_preds[j] = info['blend_ridge'].predict(blend)[0]
        preds_v12 = final_preds

    out = [None] * len(animals)
    for j, vi in enumerate(valid_idx):
        out[vi] = round(float(preds_v12[j]), 2)
    return out

# ============================================================
# LOAD CLIENT FILES — MERGE BOTH SOURCES
# ============================================================
print(f"\n{'='*120}")
print(f"  DSII V12 — PREDICAO CLIENTE DANIEL (Lineage Learning + Calibration + Shrinkage)")
print(f"{'='*120}")

# Load both files
df1 = pd.read_excel(INPUT_FILE_1)
df2 = pd.read_excel(INPUT_FILE_2)
print(f"  cliente daniel.xls: {len(df1)} animais (sid/mgs/mmgs)")
print(f"  cliente daniel_2.xls: {len(df2)} animais (Pai/Avo/Bisavo)")

# Build unified animal list: prefer daniel_2 format (has bisavo), fill from daniel_1
animals_dict = {}

# First: daniel_2.xls (ID Animal, Pai, Avo Materno, Bisavo Materno)
cols2 = list(df2.columns)
for _, row in df2.iterrows():
    aid = row[cols2[0]]
    pai = str(row[cols2[1]]).strip()
    avo = str(row[cols2[2]]).strip()
    bis = str(row[cols2[3]]).strip()
    if pai not in ('', 'nan', 'None'):
        animals_dict[aid] = {'pai': pai, 'avo': avo, 'bis': bis, 'source': 'daniel_2'}

# Second: daniel.xls (cowid, sid, mgs, mmgs) — add missing
for _, row in df1.iterrows():
    aid = row['cowid']
    if aid in animals_dict: continue
    sid = str(row.get('sid', '')).strip()
    mgs = str(row.get('mgs', '')).strip()
    mmgs = str(row.get('mmgs', '')).strip()
    if sid not in ('', 'nan', 'None'):
        animals_dict[aid] = {'pai': sid, 'avo': mgs, 'bis': mmgs, 'source': 'daniel_1'}

print(f"  Animais unicos apos merge: {len(animals_dict)}")
...
all_naabs = set()
for aid, info in animals_dict.items():
    for k in ['pai', 'avo', 'bis']:
        n = info[k]
...
for aid, info in animals_dict.items():
    ...
    if not avo_row and avo_naab not in ('', 'nan', 'None'): missing.append(f"Avo({avo_naab})")

    if not pai_row or not pai_ptas:
        excluded.append({'ID': aid, 'Pai': pai_naab, 'Avo': avo_naab, 'Bis': bis_naab,
                         'motivo': ', '.join(missing) if missing else 'Pai sem PTAs'})
        continue

    meta = {
        'ID': aid,
        'Pai_NAAB': pai_naab, 'Pai_Resolved': pai_resolved, 'Pai_Src': pai_src,
        'Avo_NAAB': avo_naab, 'Avo_Resolved': avo_resolved if avo_resolved else '',
        'Avo_Src': avo_src if avo_src else '',
        'Bis_NAAB': bis_naab, 'Bis_Resolved': bis_resolved if bis_resolved else '',
        'Bis_Src': bis_src if bis_src else '',
        'Pai_Traits': len(pai_ptas), 'Avo_Traits': len(avo_ptas),
        'Bis_Traits': len(bis_ptas),
        'Fonte': info['source'],
    }
...
for trait in ALL_TRAITS:
    if trait == 'FI':
        continue  # FI calculated from formula below
    t0 = time.time()
    preds = predict_batch(trait, animals_input)
...
# FI from formula (CDCB 2024): FI = 0.4*DPR + 0.4*CCR + 0.1*HCR + 0.1*EFC
fi_count = 0
for r in results:
    dpr = r.get('DPR'); ccr = r.get('CCR'); hcr = r.get('HCR'); efc = r.get('EFC')
    if dpr is not None and ccr is not None and hcr is not None:
        r['FI'] = round(0.4 * dpr + 0.4 * ccr + 0.1 * hcr + 0.1 * (efc if efc is not None else 0), 4)
        fi_count += 1
print(f"        FI: {fi_count:>4} calculados via formula (0.4*DPR + 0.4*CCR + 0.1*HCR + 0.1*EFC)")
sys.stdout.flush()

print(f"\n  Animais VALIDOS: {len(results)}/{len(animals_dict)} ({100*len(results)/len(animals_dict):.1f}%)")
print(f"  Animais EXCLUIDOS: {len(excluded)}/{len(animals_dict)}")
...
if results:
    ...
    meta_cols = ['ID','Pai_NAAB','Pai_Resolved','Pai_Src','Avo_NAAB','Avo_Resolved','Avo_Src',
                 'Bis_NAAB','Bis_Resolved','Bis_Src','Pai_Traits','Avo_Traits','Bis_Traits','Fonte']
    ordered_traits = []
...
print(f"\n  Predicao V12 cliente Daniel concluida!")
